...
const getOptions = function getOptions (obj) {
  const options = Array.prototype.reduce.call(obj, function (acc, attribute) {
    const option = attribute.name.match(/data-simplebar-(.+)/)

    if (option) {
      const key = option[1].replace(/\W+(.)/g, function (x, chr) {
...
(function () {
  function SimpleBar (element, options) {
    ...
    this.onScroll = function () {
      const elWindow = getElementWindow(_this.el)
    ...
    this.drag = function (e) {
      let eventOffset
      const track = _this.axis[_this.draggedAxis].track
      const trackSize = track.rect[_this.axis[_this.draggedAxis].sizeAttr]
      const scrollbar = _this.axis[_this.draggedAxis].scrollbar
      const contentSize = _this.contentWrapperEl[_this.axis[_this.draggedAxis].scrollSizeAttr]
      const hostSize = parseInt(_this.elStyles[_this.axis[_this.draggedAxis].sizeAttr], 10)
      e.preventDefault()
      e.stopPropagation()
      ...
      const dragPos = eventOffset - track.rect[_this.axis[_this.draggedAxis].offsetAttr] - _this.axis[_this.draggedAxis].dragOffset // Convert the mouse position into a percentage of the scrollbar height/width.

      const dragPerc = dragPos / (trackSize - scrollbar.size) // Scroll the content by the same percentage.
    ...
    this.onEndDrag = function (e) {
      const elDocument = getElementDocument(_this.el)
      const elWindow = getElementWindow(_this.el)
      e.preventDefault()
      e.stopPropagation()
  ...
  const _proto = SimpleBar.prototype
