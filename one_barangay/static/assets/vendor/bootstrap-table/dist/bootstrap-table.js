(function (global, factory) {
  ...
  // Nashorn ~ JDK8 bug
  const NASHORN_BUG = getOwnPropertyDescriptor$4 && !nativePropertyIsEnumerable.call({ 1: 2 }, 1)

  // `Object.prototype.propertyIsEnumerable` method implementation
  // https://tc39.es/ecma262/#sec-object.prototype.propertyisenumerable
  const f$4 = NASHORN_BUG ? function propertyIsEnumerable (V) {
    const descriptor = getOwnPropertyDescriptor$4(this, V)
  ...
  */
  ...
  const STRICT_METHOD$3 = arrayMethodIsStrict('join', ',')
  ...
  const UNSUPPORTED_Y$2 = fails(function () {
    // babel-minify transpiles RegExp('a', 'y') -> /a/y and it causes SyntaxError
    const re = RE('a', 'y')
    re.lastIndex = 2
  ...
  const UNSUPPORTED_Y$1 = regexpStickyHelpers.UNSUPPORTED_Y || regexpStickyHelpers.BROKEN_CARET
  ...
  const FORCED$4 = !IS_CONCAT_SPREADABLE_SUPPORT || !SPECIES_SUPPORT
  ...
  const trim$2 = stringTrim.trim

  const $parseFloat = global_1.parseFloat
  const FORCED$3 = 1 / $parseFloat(whitespaces + '-0') !== -Infinity

  // `parseFloat` method
  // https://tc39.es/ecma262/#sec-parsefloat-string
  const numberParseFloat = FORCED$3 ? function parseFloat (string) {
    const trimmedString = trim$2(String(string))
  ...
  const FORCED$2 = FAILS_ON_UNDEFINED || !FAILS_ON_NULL || !STRICT_METHOD
  ...
  const HAS_SPECIES_SUPPORT$3 = arrayMethodHasSpeciesSupport('filter')
  ...
  const UNSUPPORTED_Y = regexpStickyHelpers.UNSUPPORTED_Y

  const FORCED = descriptors && isForced_1('RegExp', (!CORRECT_NEW || UNSUPPORTED_Y || fails(function () {
    re2[MATCH] = false
  ...
  if (FORCED) {
    ...
    const keys$1 = getOwnPropertyNames$1(NativeRegExp)
    let index = 0
  ...
  const NEW_ITERATOR_PROTOTYPE = IteratorPrototype$2 == undefined || fails(function () {
    const test = {}
  ...
  const getInternalState = internalState.getterFor(ARRAY_ITERATOR)
  ...
  const es_array_iterator = defineIterator(Array, 'Array', function (iterated, kind) {
  ...
  try {
    const rawVersion = $__default.default.fn.dropdown.Constructor.VERSION // Only try to parse VERSION if it is defined.
  ...
  try {
    ...
    const _rawVersion = bootstrap.Tooltip.VERSION
  ...
  const CORRECT_IS_REGEXP_LOGIC$1 = correctIsRegexpLogic('startsWith')
  // https://github.com/zloirock/core-js/pull/702
  const MDN_POLYFILL_BUG$1 = !CORRECT_IS_REGEXP_LOGIC$1 && !!(function () {
    const descriptor = getOwnPropertyDescriptor$1(String.prototype, 'startsWith')
  ...
  _export({ target: 'String', proto: true, forced: !MDN_POLYFILL_BUG$1 && !CORRECT_IS_REGEXP_LOGIC$1 }, {
    startsWith: function startsWith (searchString /* , position = 0 */) {
      ...
      const index = toLength(min$1(arguments.length > 1 ? arguments[1] : undefined, that.length))
  ...
  const nativeEndsWith = ''.endsWith
  const min = Math.min

  const CORRECT_IS_REGEXP_LOGIC = correctIsRegexpLogic('endsWith')
  // https://github.com/zloirock/core-js/pull/702
  const MDN_POLYFILL_BUG = !CORRECT_IS_REGEXP_LOGIC && !!(function () {
  ...
  const Utils = {
    ...
    sprintf: function sprintf (_str) {
      ...
      const str = _str.replace(/%s/g, function () {
        const arg = args[i++]
    ...
    getFieldTitle: function getFieldTitle (list, value) {
      const _iterator = _createForOfIteratorHelper(list)
      let _step
    ...
    setFieldIndex: function setFieldIndex (columns) {
      ...
      const _iterator2 = _createForOfIteratorHelper(columns[0])
      let _step2
      ...
      for (let _i = 0; _i < columns.length; _i++) {
        const _iterator3 = _createForOfIteratorHelper(columns[_i])
        var _step3

        try {
          for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
            const r = _step3.value
            const rowspan = r.rowspan || 1
            const colspan = r.colspan || 1

            const index = flag[_i].indexOf(false)
    ...
    updateFieldGroup: function updateFieldGroup (columns) {
      ...
      const allColumns = (_ref = []).concat.apply(_ref, _toConsumableArray(columns))

      const _iterator4 = _createForOfIteratorHelper(columns)
      let _step4

      try {
        for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
          const c = _step4.value

          const _iterator5 = _createForOfIteratorHelper(c)
          var _step5
            ...
            for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
              ...
              if (r.colspanGroup > 1) {
                ...
                const _loop = function _loop (i) {
                  const column = allColumns.find(function (col) {
    ...
    getScrollBarWidth: function getScrollBarWidth () {
      if (this.cachedWidth === undefined) {
        const $inner = $__default.default('<div/>').addClass('fixed-table-scroll-inner')
        const $outer = $__default.default('<div/>').addClass('fixed-table-scroll-outer')
        $outer.append($inner)
    ...
    calculateObjectValue: function calculateObjectValue (self, name, args, defaultValue) {
      ...
      if (typeof name === 'string') {
        ...
        if (names.length > 1) {
          ...
          const _iterator6 = _createForOfIteratorHelper(names)
          let _step6

          try {
            for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
              const f = _step6.value
              func = func[f]
            }
          } catch (err) {
            _iterator6.e(err)
    ...
    compareObjects: function compareObjects (objectA, objectB, compareLength) {
      ...
      for (let _i2 = 0, _aKeys = aKeys; _i2 < _aKeys.length; _i2++) {
        const key = _aKeys[_i2]
    ...
    getRealDataAttr: function getRealDataAttr (dataAttr) {
      for (let _i3 = 0, _Object$entries = Object.entries(dataAttr); _i3 < _Object$entries.length; _i3++) {
        const _Object$entries$_i = _slicedToArray(_Object$entries[_i3], 2)
        const attr = _Object$entries$_i[0]
    ...
    getItemField: function getItemField (item, field, escape) {
      ...
      const _iterator7 = _createForOfIteratorHelper(props)
      let _step7
    ...
    findIndex: function findIndex (items, item) {
      const _iterator8 = _createForOfIteratorHelper(items)
      let _step8

      try {
        for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
          const it = _step8.value
    ...
    trToData: function trToData (columns, $els) {
      ...
      $els.each(function (y, el) {
        ...
        const row = {} // save tr's id, class and data-* attributes
        ...
        $el.find('>td,>th').each(function (_x, el) {
          ...
          let x = _x // skip already occupied cells in current row
    ...
    checkAutoMergeCells: function checkAutoMergeCells (data) {
      const _iterator9 = _createForOfIteratorHelper(data)
      let _step9
      ...
      try {
        for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
          ...
          for (let _i4 = 0, _Object$keys = Object.keys(row); _i4 < _Object$keys.length; _i4++) {
            const key = _Object$keys[_i4]
  ...
  const BootstrapTable = /* #__PURE__ */(function () {
    ...
    _createClass(BootstrapTable, [{
      ...
      value: function initConstants () {
        ...
        const buttonsPrefix = opts.buttonsPrefix ? ''.concat(opts.buttonsPrefix, '-') : ''
      ...
      value: function initLocale () {
        if (this.options.locale) {
          const locales = $__default.default.fn.bootstrapTable.locales
      ...
      value: function initContainer () {
        ...
        const loadingTemplate = Utils.calculateObjectValue(this.options, this.options.loadingTemplate, [this.options.formatLoadingMessage()])
      ...
      value: function initTable () {
        ...
        this.$header.find('tr').each(function (i, el) {
          const $tr = $__default.default(el)
          const column = []
          $tr.find('th').each(function (i, el) {
            const $th = $__default.default(el) // #2014: getFieldIndex and elsewhere assume this is string, causes issues if not
        ...
        this.options.columns.forEach(function (columns, i) {
          columns.forEach(function (_column, j) {
            const column = $__default.default.extend({}, BootstrapTable.COLUMN_DEFAULTS, _column)
      ...
      value: function initHeader () {
        ...
        this.options.columns.forEach(function (columns, i) {
          ...
          if (i === 0 && Utils.hasDetailViewIcon(_this2.options)) {
            const rowspan = _this2.options.columns.length > 1 ? ' rowspan="'.concat(_this2.options.columns.length, '"') : ''
          ...
          columns.forEach(function (column, j) {
            ...
            const headerStyle = Utils.calculateObjectValue(null, _this2.options.headerStyle, [column])
            const csses = []
            let classes = ''

            if (headerStyle && headerStyle.css) {
              for (let _i = 0, _Object$entries = Object.entries(headerStyle.css); _i < _Object$entries.length; _i++) {
            ...
            let text = _this2.options.escape ? Utils.escapeHTML(column.title) : column.title
            const title = text
        ...
        this.$container.off('click', '.th-inner').on('click', '.th-inner', function (e) {
          const $this = $__default.default(e.currentTarget)
        ...
        this.$selectAll.off('click').on('click', function (e) {
          e.stopPropagation()
          const checked = $__default.default(e.currentTarget).prop('checked')
        ...
        if (index !== -1) {
          ...
          if (this.options.customSort) {
            ...
            this.data.sort(function (a, b) {
              ...
              const aa = Utils.getItemField(a, name, _this3.options.escape)
              const bb = Utils.getItemField(b, name, _this3.options.escape)
              const value = Utils.calculateObjectValue(_this3.header, _this3.header.sorters[index], [aa, bb, a, b])
          ...
          if (this.options.sortClass !== undefined) {
            ...
            timeoutId = setTimeout(function () {
              ...
              const index = _this3.$header.find('[data-field="'.concat(_this3.options.sortName, '"]')).index()
      ...
      value: function onSort (_ref) {
        ...
        const currentTarget = _ref.currentTarget
        const $this = type === 'keypress' ? $__default.default(currentTarget) : $__default.default(currentTarget).parent()
      ...
      value: function initToolbar () {
          ...
          columns: {
            ...
            html: function html () {
              ...
              if (opts.showColumnsToggleAll) {
                const allFieldsVisible = _this4.getVisibleColumns().length === _this4.columns.filter(function (column) {
        ...
        for (let _i2 = 0, _Object$entries2 = Object.entries(this.buttons); _i2 < _Object$entries2.length; _i2++) {
          const _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2)
          const buttonName = _Object$entries2$_i[0]
          const buttonConfig = _Object$entries2$_i[1]

          let buttonHtml = void 0
          ...
          if (buttonConfig.hasOwnProperty('html')) {
            ...
            if (buttonConfig.hasOwnProperty('attributes')) {
              for (let _i3 = 0, _Object$entries3 = Object.entries(buttonConfig.attributes); _i3 < _Object$entries3.length; _i3++) {
                const _Object$entries3$_i = _slicedToArray(_Object$entries3[_i3], 2)
                const attributeName = _Object$entries3$_i[0]
                const value = _Object$entries3$_i[1]
          ...
          const optionName = 'show'.concat(buttonName.charAt(0).toUpperCase()).concat(buttonName.substring(1))
        ...
        const _iterator = _createForOfIteratorHelper(opts.buttonsOrder)
        let _step

        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            const button = _step.value
            const _showOption = opts['show'.concat(button.charAt(0).toUpperCase()).concat(button.substring(1))]
        ...
        for (let _i4 = 0, _Object$entries4 = Object.entries(this.buttons); _i4 < _Object$entries4.length; _i4++) {
          const _Object$entries4$_i = _slicedToArray(_Object$entries4[_i4], 2)
          var _buttonName = _Object$entries4$_i[0]
          var _buttonConfig = _Object$entries4$_i[1]

          if (_buttonConfig.hasOwnProperty('event')) {
            if (typeof _buttonConfig.event === 'function' || typeof _buttonConfig.event === 'string') {
              const _ret = (function () {
                const event = typeof _buttonConfig.event === 'string' ? window[_buttonConfig.event] : _buttonConfig.event
            ...
            const _loop = function _loop () {
              ...
              const eventType = _Object$entries5$_i[0]
              const eventFunction = _Object$entries5$_i[1]
        ...
        if (opts.showColumns) {
          ...
          $checkboxes.off('click').on('click', function (_ref2) {
            const currentTarget = _ref2.currentTarget
            const $this = $__default.default(currentTarget)
          ...
          $toggleAll.off('click').on('click', function (_ref3) {
            const currentTarget = _ref3.currentTarget
          ...
          if (opts.showColumnsSearch) {
            ...
            $columnsSearch.on('keyup paste change', function (_ref4) {
              ...
              $checkboxes.each(function (i, el) {
                const $checkbox = $__default.default(el)
        ...
        const handleInputEvent = function handleInputEvent ($searchInput) {
        ...
        if ((opts.search || this.showSearchClearButton) && typeof opts.searchSelector !== 'string') {
          html = []
          const showSearchButton = Utils.sprintf(this.constants.html.searchButton, this.constants.buttonsClass, opts.formatSearch(), opts.showButtonIcons ? Utils.sprintf(this.constants.html.icon, opts.iconsPrefix, opts.icons.search) : '', opts.showButtonText ? opts.formatSearch() : '')
          const showSearchClearButton = Utils.sprintf(this.constants.html.searchClearButton, this.constants.buttonsClass, opts.formatClearSearch(), opts.showButtonIcons ? Utils.sprintf(this.constants.html.icon, opts.iconsPrefix, opts.icons.clearSearch) : '', opts.showButtonText ? opts.formatClearSearch() : '')
          const searchInputHtml = '<input class="'.concat(this.constants.classes.input, '\n        ').concat(Utils.sprintf(' %s%s', this.constants.classes.inputPrefix, opts.iconSize), '\n        search-input" type="search" placeholder="').concat(opts.formatSearch(), '" autocomplete="off">')
          let searchInputFinalHtml = searchInputHtml

          if (opts.showSearchButton || opts.showSearchClearButton) {
            const _buttonsHtml = (opts.showSearchButton ? showSearchButton : '') + (opts.showSearchClearButton ? showSearchClearButton : '')
          ...
          const _$searchInput = Utils.getSearchInput(this)
      ...
      value: function onSearch () {
        ...
        const firedByInitSearchText = _ref5.firedByInitSearchText

        const overwriteSearchText = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true

        if (currentTarget !== undefined && $__default.default(currentTarget).length && overwriteSearchText) {
          const text = $__default.default(currentTarget).val().trim()
        ...
        if (this.options.sidePagination !== 'server') {
          ...
          if (typeof this.filterOptions.filterAlgorithm === 'function') {
            ...
            this.data = f ? this.options.data.filter(function (item) {
              const filterAlgorithm = _this5.filterOptions.filterAlgorithm
          ...
          this.data = s ? this.data.filter(function (item, i) {
            for (let j = 0; j < _this5.header.fields.length; j++) {
              if (!_this5.header.searchables[j] || _this5.options.visibleSearch && visibleFields.indexOf(_this5.header.fields[j]) === -1) {
                continue
              }

              const key = Utils.isNumeric(_this5.header.fields[j]) ? parseInt(_this5.header.fields[j], 10) : _this5.header.fields[j]
              const column = _this5.columns[_this5.fieldsColumnsIndex[key]]
              let value = void 0
              ...
              if (typeof key === 'string') {
                ...
                for (let _i6 = 0; _i6 < props.length; _i6++) {
              ...
              if (typeof value === 'string' || typeof value === 'number') {
                if (_this5.options.strictSearch) {
                  ...
                  const matches = largerSmallerEqualsRegex.exec(_this5.searchText)
                  let comparisonCheck = false

                  if (matches) {
                    const operator = matches[1] || ''.concat(matches[5], 'l')
      ...
      value: function initPagination () {
        ...
        if (this.paginationParts.includes('pageSize')) {
          ...
          const pageNumber = ['<div class="'.concat(this.constants.classes.paginationDropdown, '">\n        <button class="').concat(this.constants.buttonsClass, ' dropdown-toggle" type="button" ').concat(this.constants.dataToggle, '="dropdown">\n        <span class="page-size">\n        ').concat(allSelected ? opts.formatAllRows() : opts.pageSize, '\n        </span>\n        ').concat(this.constants.html.dropdownCaret, '\n        </button>\n        ').concat(this.constants.html.pageDropdown[0])]
        ...
        if (this.paginationParts.includes('pageList')) {
          ...
          const pageItem = function pageItem (i) {
            const classes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ''
        ...
        const dropupClass = ['bottom', 'both'].includes(opts.paginationVAlign) ? ' '.concat(this.constants.classes.dropup) : ''
      ...
      value: function initRow (item, i, data, trFragments) {
        ...
        if (style && style.css) {
          for (let _i7 = 0, _Object$entries6 = Object.entries(style.css); _i7 < _Object$entries6.length; _i7++) {
            const _Object$entries6$_i = _slicedToArray(_Object$entries6[_i7], 2)
            const key = _Object$entries6$_i[0]
            const value = _Object$entries6$_i[1]
        ...
        if (attributes) {
          for (let _i8 = 0, _Object$entries7 = Object.entries(attributes); _i8 < _Object$entries7.length; _i8++) {
            const _Object$entries7$_i = _slicedToArray(_Object$entries7[_i8], 2)
            const _key2 = _Object$entries7$_i[0]
            const _value = _Object$entries7$_i[1]
        ...
        if (item._data && !Utils.isEmptyObject(item._data)) {
          for (let _i9 = 0, _Object$entries8 = Object.entries(item._data); _i9 < _Object$entries8.length; _i9++) {
            const _Object$entries8$_i = _slicedToArray(_Object$entries8[_i9], 2)
            const k = _Object$entries8$_i[0]
            const v = _Object$entries8$_i[1]
        ...
        this.header.fields.forEach(function (field, j) {
          let text = ''
          let value_ = Utils.getItemField(item, field, _this7.options.escape)
          let value = ''
          let type = ''
          let cellStyle = {}
          let id_ = ''
          let class_ = _this7.header.classes[j]
          let style_ = ''
          let styleToAdd_ = ''
          let data_ = ''
          let rowspan_ = ''
          ...
          if (cellStyle.css) {
            ...
            for (let _i10 = 0, _Object$entries9 = Object.entries(cellStyle.css); _i10 < _Object$entries9.length; _i10++) {
              const _Object$entries9$_i = _slicedToArray(_Object$entries9[_i10], 2)
              const _key3 = _Object$entries9$_i[0]
              const _value2 = _Object$entries9$_i[1]
          ...
          if (column.searchable && _this7.searchText && _this7.options.searchHighlight) {
            let defValue = ''
            const regExp = new RegExp('('.concat(_this7.searchText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), ')'), 'gim')
          ...
          if (item['_'.concat(field, '_data')] && !Utils.isEmptyObject(item['_'.concat(field, '_data')])) {
            for (let _i11 = 0, _Object$entries10 = Object.entries(item['_'.concat(field, '_data')]); _i11 < _Object$entries10.length; _i11++) {
              const _Object$entries10$_i = _slicedToArray(_Object$entries10[_i11], 2)
              const _k = _Object$entries10$_i[0]
              const _v = _Object$entries10$_i[1]
          ...
          if (column.checkbox || column.radio) {
            ...
            const isChecked = Utils.isObject(value) && value.hasOwnProperty('checked') ? value.checked : (value === true || value_) && value !== false
            ...
            const cardTitle = _this7.options.showHeader ? '<span class="card-view-title '.concat(cellStyle.classes, '"').concat(style_, '>').concat(Utils.getFieldTitle(_this7.columns, field), '</span>') : ''
      ...
      value: function initBody (fixedScroll) {
        ...
        const rows = []
        const trFragments = $__default.default(document.createDocumentFragment())
        let hasTr = false
      ...
      value: function initBodyEvent () {
        ...
        this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', function (e) {
          const $td = $__default.default(e.currentTarget)
          const $tr = $td.parent()
          const $cardViewArr = $__default.default(e.target).parents('.card-views').children()
          const $cardViewTarget = $__default.default(e.target).parents('.card-view')
          const rowIndex = $tr.data('index')
          const item = _this9.data[rowIndex]
          const index = _this9.options.cardView ? $cardViewArr.index($cardViewTarget) : $td[0].cellIndex

          const fields = _this9.getVisibleFields()

          const field = fields[index - Utils.getDetailViewIndexOffset(_this9.options)]
          const column = _this9.columns[_this9.fieldsColumnsIndex[field]]
          const value = Utils.getItemField(item, field, _this9.options.escape)
          ...
          if (e.type === 'click' && _this9.options.clickToSelect && column.clickToSelect && !Utils.calculateObjectValue(_this9.options, _this9.options.ignoreClickToSelectOn, [e.target])) {
            const $selectItem = $tr.find(Utils.sprintf('[name="%s"]', _this9.options.selectItemName))
        ...
        this.header.events.forEach(function (_events, i) {
          ...
          const field = _this9.header.fields[i]

          let fieldIndex = _this9.getVisibleFields().indexOf(field)
          ...
          const _loop2 = function _loop2 (key) {
            ...
            _this9.$body.find('>tr:not(.no-records-found)').each(function (i, tr) {
              ...
              const $td = $tr.find(_this9.options.cardView ? '.card-views>.card-view' : '>td').eq(fieldIndex)
              const index = key.indexOf(' ')
              const name = key.substring(0, index)
              const el = key.substring(index + 1)
              $td.find(el).off(name).on(name, function (e) {
                const index = $tr.data('index')
                const row = _this9.data[index]
      ...
      value: function initServer (silent, query, url) {
        ...
        if (this.options.search && this.options.sidePagination === 'server' && this.columns.filter(function (column) {
          ...
          const _iterator2 = _createForOfIteratorHelper(this.columns)
          let _step2
        ...
        const request = $__default.default.extend({}, Utils.calculateObjectValue(null, this.options.ajaxOptions), {
          ...
          success: function success (_res, textStatus, jqXHR) {
            const res = Utils.calculateObjectValue(_this10.options, _this10.options.responseHandler, [_res, jqXHR], _res)
      ...
      value: function resetRows () {
        const _iterator3 = _createForOfIteratorHelper(this.data)
        let _step3
      ...
      value: function trigger (_name) {
        let _this$options, _this$options2

        const name = ''.concat(_name, '.bs.table')
      ...
      value: function fitHeader () {
        ...
        const scrollWidth = fixedBody.scrollWidth > fixedBody.clientWidth && fixedBody.scrollHeight > fixedBody.clientHeight + this.$header.outerHeight() ? Utils.getScrollBarWidth() : 0
        ...
        if (focused.length > 0) {
          ...
          if ($th.length > 0) {
            ...
            if (dataField !== undefined) {
              const $headerTh = this.$header.find("[data-field='".concat(dataField, "']"))
        ...
        const focusedTemp = $__default.default('.focus-temp:visible:eq(0)')
        ...
        $tr.find('> *').each(function (i, el) {
          ...
          if (Utils.hasDetailViewIcon(_this14.options)) {
            if (i === 0 && _this14.options.detailViewAlign !== 'right' || i === trLength - 1 && _this14.options.detailViewAlign === 'right') {
              ...
              const _zoomWidth = $thDetail.innerWidth() - $thDetail.find('.fht-cell').width()
          ...
          const index = i - Utils.getDetailViewIndexOffset(_this14.options)

          let $th = _this14.$header_.find(Utils.sprintf('th[data-field="%s"]', visibleFields[index]))
      ...
      value: function initFooter () {
        ...
        const _iterator4 = _createForOfIteratorHelper(this.columns)
        let _step4

        try {
          for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
            const column = _step4.value
            let falign = ''
            let valign = ''
            const csses = []
            let style = {}
            ...
            if (style && style.css) {
              for (let _i12 = 0, _Object$entries11 = Object.entries(style.css); _i12 < _Object$entries11.length; _i12++) {
                const _Object$entries11$_i = _slicedToArray(_Object$entries11[_i12], 2)
                const key = _Object$entries11$_i[0]
                const _value3 = _Object$entries11$_i[1]
        ...
        $tr.find('> *').each(function (i, el) {
          ...
          if (Utils.hasDetailViewIcon(_this15.options)) {
            if (i === 0 && _this15.options.detailViewAlign === 'left' || i === trLength - 1 && _this15.options.detailViewAlign === 'right') {
              ...
              const _zoomWidth2 = $thDetail.innerWidth() - $thDetail.find('.fht-cell').width()
      ...
      value: function horizontalScroll () {
        ...
        this.$tableBody.off('scroll').on('scroll', function () {
          const scrollLeft = _this16.$tableBody.scrollLeft()
      ...
      value: function getVisibleFields () {
        ...
        const _iterator5 = _createForOfIteratorHelper(this.header.fields)
        let _step5
      ...
      value: function getOptions () {
        ...
        const options = $__default.default.extend({}, this.options)
        delete options.data
      ...
      value: function getData (params) {
        ...
        if (params && params.formatted) {
          data.forEach(function (row) {
            for (let _i13 = 0, _Object$entries12 = Object.entries(row); _i13 < _Object$entries12.length; _i13++) {
              const _Object$entries12$_i = _slicedToArray(_Object$entries12[_i13], 2)
              const key = _Object$entries12$_i[0]
              const value = _Object$entries12$_i[1]

              const column = _this17.columns[_this17.fieldsColumnsIndex[key]]
      ...
      value: function load (_data) {
        ...
        let data = _data // #431: support pagination
      ...
      value: function updateRow (params) {
        ...
        const _iterator6 = _createForOfIteratorHelper(allParams)
        let _step6

        try {
          for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
            const _params = _step6.value
      ...
      value: function updateByUniqueId (params) {
        ...
        const _iterator7 = _createForOfIteratorHelper(allParams)
        let _step7

        try {
          for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
            const _params2 = _step7.value

            if (!_params2.hasOwnProperty('id') || !_params2.hasOwnProperty('row')) {
              continue
            }

            const rowId = this.options.data.indexOf(this.getRowByUniqueId(_params2.id))
      ...
      value: function updateCellByUniqueId (params) {
        ...
        allParams.forEach(function (_ref6) {
          const id = _ref6.id
          const field = _ref6.field
          const value = _ref6.value

          const rowId = _this19.options.data.indexOf(_this19.getRowByUniqueId(id))
      ...
      value: function getHiddenRows (show) {
        ...
        const _iterator8 = _createForOfIteratorHelper(data)
        let _step8
      ...
      value: function _toggleAllColumns (visible) {
        ...
        const _iterator9 = _createForOfIteratorHelper(this.columns.slice().reverse())
        let _step9

        try {
          for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
            const column = _step9.value
      ...
      value: function _toggleCheck (checked, index) {
        const $el = this.$selectItem.filter('[data-index="'.concat(index, '"]'))
        const row = this.data[index]

        if ($el.is(':radio') || this.options.singleSelect || this.options.multipleSelectRow && !this.multipleSelectRowCtrlKey && !this.multipleSelectRowShiftKey) {
          const _iterator10 = _createForOfIteratorHelper(this.options.data)
          let _step10

          try {
            for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
              const r = _step10.value
        ...
        if (this.options.multipleSelectRow) {
          if (this.multipleSelectRowShiftKey && this.multipleSelectRowLastSelectedIndex >= 0) {
            const _ref8 = this.multipleSelectRowLastSelectedIndex < index ? [this.multipleSelectRowLastSelectedIndex, index] : [index, this.multipleSelectRowLastSelectedIndex]
            const _ref9 = _slicedToArray(_ref8, 2)
            const fromIndex = _ref9[0]
            const toIndex = _ref9[1]
      ...
      value: function _toggleCheckBy (checked, obj) {
        ...
        this.data.forEach(function (row, i) {
          ...
          if (obj.values.includes(row[obj.field])) {
            let $el = _this24.$selectItem.filter(':enabled').filter(Utils.sprintf('[data-index="%s"]', i))
  ...
  $__default.default.fn.bootstrapTable = function (option) {
    ...
    this.each(function (i, el) {
      let data = $__default.default(el).data('bootstrap.table')
      const options = $__default.default.extend({}, BootstrapTable.DEFAULTS, $__default.default(el).data(), _typeof(option) === 'object' && option)
