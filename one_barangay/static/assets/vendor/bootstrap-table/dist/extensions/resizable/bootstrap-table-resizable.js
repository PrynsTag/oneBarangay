(function (global, factory) {
  ...
  */
  ...
  const isInit = function isInit (that) {
    return that.$el.data('resizableColumns') !== undefined
  }

  const initResizable = function initResizable (that) {
  ...
  const destroy = function destroy (that) {
    if (isInit(that)) {
      that.$el.data('resizableColumns').destroy()
    }
  }

  const reInitResizable = function reInitResizable (that) {
    destroy(that)
    initResizable(that)
  }
