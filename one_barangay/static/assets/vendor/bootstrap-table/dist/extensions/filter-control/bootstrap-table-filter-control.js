(function (global, factory) {
  ...
  */
  ...
  const FORCED$2 = !IS_CONCAT_SPREADABLE_SUPPORT || !SPECIES_SUPPORT
  ...
  const STRICT_METHOD$3 = arrayMethodIsStrict('forEach')

  // `Array.prototype.forEach` method implementation
  // https://tc39.es/ecma262/#sec-array.prototype.foreach
  const arrayForEach = !STRICT_METHOD$3 ? function forEach (callbackfn /* , thisArg */) {
  ...
  const Utils$1 = $__default.default.fn.bootstrapTable.utils
    ...
    for (let i = 0; i < options.length; i++) {
      if (options[i].value !== '') {
        if (!uniqueValues.hasOwnProperty(options[i].value)) {
          selectControl.find(Utils$1.sprintf('option[value=\'%s\']', options[i].value)).hide()
        } else {
          selectControl.find(Utils$1.sprintf('option[value=\'%s\']', options[i].value)).show()
        }
      }
    }
  }
  ...
  function initFilterSelectControls (that) {
    ...
    $__default.default.each(that.header.fields, function (j, field) {
      ...
      if (isColumnSearchableViaSelect(column) && isFilterDataNotGiven(column) && hasSelectControlElement(selectControl)) {
        ...
        for (let i = 0; i < z; i++) {
          // Added a new value
          let fieldValue = Utils$1.getItemField(data[i], field, false)
          const formatter = that.options.editable && column.editable ? column._formatter : that.header.formatters[j]
          let formattedValue = Utils$1.calculateObjectValue(that.header, formatter, [fieldValue, data[i], i], fieldValue)
  ...
  $__default.default.extend($__default.default.fn.bootstrapTable.defaults, {
    ...
    filterTemplate: {
      input: function input (that, field, placeholder, value) {
        return Utils.sprintf('<input type="search" class="form-control bootstrap-table-filter-control-%s search-input" style="width: 100%;" placeholder="%s" value="%s">', field, typeof placeholder === 'undefined' ? '' : placeholder, typeof value === 'undefined' ? '' : value)
      },
      ...
      datepicker: function datepicker (that, field, value) {
        return Utils.sprintf('<input type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s" style="width: 100%;" value="%s">', field, typeof value === 'undefined' ? '' : value)
      }
    },
  ...
  $__default.default.BootstrapTable = /* #__PURE__ */(function (_$$BootstrapTable) {
    ...
    _createClass(_class, [{
      ...
      value: function init () {
        ...
        if (this.options.filterControl) {
          ...
          this.$el.on('reset-view.bs.table', function () {
            ...
            const $controlContainer = getControlContainer(_this)
      ...
      value: function initSearch () {
        ...
        const that = this
        const fp = $__default.default.isEmptyObject(that.filterColumnsPartial) ? null : that.filterColumnsPartial
        ...
        that.data = fp ? that.data.filter(function (item, i) {
          ...
          const keys = keys1.concat(keys2.filter(function (item) {
      ...
      value: function onColumnSearch (_ref2) {
        ...
        copyValues(this)
        const text = $__default.default.trim($__default.default(currentTarget).val())
        const $field = $__default.default(currentTarget).closest('[data-field]').data('field')
      ...
      value: function clearFilterControl () {
        if (this.options.filterControl) {
          const that = this
          const cookies = collectBootstrapCookies()
          ...
          if (that.options.sortName !== table.data('sortName') || that.options.sortOrder !== table.data('sortOrder')) {
            const sorter = this.$header.find(Utils.sprintf('[data-field="%s"]', $__default.default(controls[0]).closest('table').data('sortName')))
      ...
      value: function triggerSearch () {
        const searchControls = getSearchControls(this)
        searchControls.each(function () {
          const el = $__default.default(this)
      ...
      value: function toggleFilterControl () {
        ...
        const $filterControls = getControlContainer(this).find('.filter-control, .no-filter-control')
