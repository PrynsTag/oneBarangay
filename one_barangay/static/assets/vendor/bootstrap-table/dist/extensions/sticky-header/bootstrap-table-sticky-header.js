      ...
      value: function renderStickyHeader () {
        ...
        if (this.options.filterControl) {
          $__default.default(this.$stickyHeader).off('keyup change mouseup').on('keyup change mouse', function (e) {
            ...
            const $coreTh = that.$header.find('th[data-field="'.concat(field, '"]'))
        ...
        const top = $__default.default(window).scrollTop() // top anchor scroll position, minus header height

        const start = this.$stickyBegin.offset().top - this.options.stickyHeaderOffsetY // bottom anchor scroll position, minus header height, minus sticky height

        const end = this.$stickyEnd.offset().top - this.options.stickyHeaderOffsetY - this.$header.height() // show sticky when top anchor touches header, and when bottom anchor not exceeded
