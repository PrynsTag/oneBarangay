(function (global, factory) {
  ...
  */
  ...
  const FORCED$1 = descriptors && isForced_1('RegExp', (!CORRECT_NEW || UNSUPPORTED_Y || fails(function () {
    re2[MATCH] = false
  ...
  function _GET (key) {
    const url = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : window.location.search
     ...
     * ...当然, 我并不知道这种用法.
     */
    const reg = new RegExp('(^|&)'.concat(key, '=([^&]*)(&|$)'))
  ...
  function _buildUrl (dict) {
    ...
    for (let _i = 0, _Object$entries = Object.entries(dict); _i < _Object$entries.length; _i++) {
      const _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2)
      const key = _Object$entries$_i[0]
      const val = _Object$entries$_i[1]

      // 搜索name=general这种形式的字符串(&是分隔符)
      const pattern = ''.concat(key, '=([^&]*)')
      const targetStr = ''.concat(key, '=').concat(val)
      ...
      if (url.match(pattern)) {
        const tmp = new RegExp('('.concat(key, '=)([^&]*)'), 'gi')
